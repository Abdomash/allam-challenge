...
class Prompter:
    def __init__(self, qafiya=None, poet=None, wazn=None):
        ...
        LOGGER.info(f"Prompter initialized with qafiya: {qafiya}, poet: {poet}, wazn: {wazn}")
    ...
    def wrap_gen(self, prompt, previous_shatrs=None, feedback=None, current_attempt=None, multi_gen=False):
        ...
        if feedback: #feedback: list of {"bayt":[s0,s1], "feedback":"str"}
            for f in feedback:
                ...
                full_text += '[INST] '
                full_text += "البيت الآخير من القصيدة قد يحتاج إلى تغيير. هذه توجيهات من محلل شعري قام بنقد هذا البيت واستخرج نقاط ممكن أن تحسن البيت من حيث معاني الكلمات والتصوير والمجازات: " #user suggested things
                full_text += "\n'"
        ...
        LOGGER.info(f"Prompt Generated by Prompter: {full_text}")
        return full_text
    
    def update(self, qafiya=None, poet=None, wazn=None):
        if poet: # in arabic
            if poet not in self.poets.keys():
                LOGGER.error(f"Poet not found in database: Could not find {poet} in 'poet.json'")
                # poet = random.choice(list(self.poets.keys()))
                poet = "المتنبي"
                LOGGER.error(f"Using a random poet instead. {poet}")
        ...
        if wazn: # in arabic
            ...
            if wazn not in self.bohours.keys():
                LOGGER.error(f"Bahr not found in database: Could not find {wazn} in 'bohours.json'")
                # wazn = random.choice(list(self.bohours.keys()))
                wazn = "الكامل"
                LOGGER.error(f"Using a random bahr instead. {wazn}")
            ...
            try:
                ...
                if len(self.poems) == 0: # allam-poet doesnt have any poems yet. We will fill them in soon.
                    if poet == "علام":
                        LOGGER.error("Allam-poet doesn't have any poems in this wazn.")
                    raise KeyError()
            except KeyError:
                LOGGER.error(f"Poet {self.poet['name']} does not have any poems in {wazn}")
                self.poems = [item for sublist in self.poet['poems'].values() for item in sublist]
        elif self.wazn is None:
            self.poems = [item for sublist in self.poet['poems'].values() for item in sublist]

        # cycle through all poem lines infinitely
